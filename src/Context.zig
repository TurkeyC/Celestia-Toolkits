...
pub const Context = struct {
    ...
    fn handleMouse(self: *Self, mouse: vaxis.Mouse) !void {
        defer self.mouse = mouse;

        switch (mouse.button) {
            .wheel_up, .wheel_down => {
                const now = std.time.milliTimestamp();
                if (now - self.last_scroll_ms < 80) return;
                self.last_scroll_ms = now;

                if (mouse.button == .wheel_up) {
                    if (mouse.mods.ctrl) {
                        self.document_handler.zoomIn(@sqrt(self.config.general.zoom_step));
                        self.reload_page = true;
                    } else if (mouse.mods.shift) {
                        self.document_handler.scroll(.Up, 1.0);
                        self.reload_page = true;
                    } else {
                        if (self.document_handler.changePage(-1)) self.resetCurrentPage();
                    }
                } else {
                    if (mouse.mods.ctrl) {
                        self.document_handler.zoomOut(@sqrt(self.config.general.zoom_step));
                        self.reload_page = true;
                    } else if (mouse.mods.shift) {
                        self.document_handler.scroll(.Down, 1.0);
                        self.reload_page = true;
                    } else {
                        if (self.document_handler.changePage(1)) self.resetCurrentPage();
                    }
            ...
            .left => {
                if (mouse.type == .drag) {
                    if (self.mouse) |prev| {
                        ...
                        const delta_col = @as(i32, mouse.col) - @as(i32, prev.col);
                        const delta_row = @as(i32, mouse.row) - @as(i32, prev.row);

                        const active_zoom = self.document_handler.getActiveZoom();
                        if (active_zoom > 0) {
                            const dx = @as(f32, @floatFromInt(delta_col)) * @as(f32, @floatFromInt(pix_per_col)) / active_zoom;
                            const dy = @as(f32, @floatFromInt(delta_row)) * @as(f32, @floatFromInt(pix_per_row)) / active_zoom;
                            self.document_handler.offsetScroll(dx, dy);
                            self.reload_page = true;
                        }
                    }
                }
